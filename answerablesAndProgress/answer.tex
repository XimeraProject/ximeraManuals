...
\title{Enter math answers with \symbol{`\\}\symbol{`\\}answer}
\author{Bart Snapp}
...
The basic way of including an answerable item in Ximera is to use the
\verb|\answer| command.
...
Compute:
\[
\frac{\partial}{\partial x} \sin(3xyz) 
= \answer{3yz\cos(3xyz)}
\]
\end{exercise}
\end{verbatim}
...
\begin{warning}
  ...
  \begin{description}
    ...
    \item[Right-click reveals the answer] to the students. Moreover, the source
      ...
      to decide equality between expressions that involving integers, $\pi$,
...
submissions as correct}\footnote{Note that it typically requires pretty 
  obscur edge cases for this to occur. This means that, in practice, this isn't something
  most authors need worry about, but it is something they should be aware of.}. 
This means that at least one of the validators
...
true.  This is because $10^{-11}$ is a number that is too small for machine
