...
\verb!question!, \verb!remark!, and a \verb!problem!.  Each of these is worth
...
\begin{description}
    \item[Theorem environment] Worth 25\% contingent on completion.
        \begin{description}
            \item[No answerables] Automatically flagged as complete upon page load.
        \end{description}
...
\[
    \begin{array}{lcc}
        ...
        \text{Theorem}     &       & 25\%\\
        \text{Question}    &       &     \\
        \bullet~\text{Select all}   & 50\%  & 12.5\%\\
        \bullet~\text{Sub-problem} &       &     \\ 
        \phantom{\bullet}~\text{--~Answer box} & 50\%  & 12.5\%\\
        \text{Remark}      &       & 25\%\\
        \text{Problem}     &       &     \\         
        \bullet~\text{Answer box}   & 100\% & 25\%\\
    \end{array}
\]
...
\begin{enumerate}
    ...
    \item Since credit is split evenly
          ...
          of parts, nested or not, can end up with individual problems or answerables that are worth
...
regardless of how many problems or answerables are in that activity.
