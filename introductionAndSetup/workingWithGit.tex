...
\pdfOnly{\begin{multicols*}{2}}
    ...
    \begin{image}
        \begin{tikzpicture}[line cap=round, line width=2.5pt]
            % Connecting lines (branches)
            \draw (0,2) -- (0,1);         % Vertical line
            \draw[smooth] plot coordinates {(0,1) (0.1,1.35) (0.4,1.4) (0.5,1.7)};         % Diagonal branch
           % Nodes (commits)
           \draw[fill=white] (0,2) circle (4pt); % Top node
           \draw[fill=white] (0,1) circle (4pt); % Middle node
           \draw[fill=white] (.5,1.7) circle (4pt); % Bottom right node
         \end{tikzpicture}
        \end{image}
